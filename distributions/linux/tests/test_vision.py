...
class TestVisionProcessor:
    ...
    def test_extract_title_from_image_openai_success(self, mock_openai):
        ...
        try:
            ...
            try:
                ...
                # Windows file permission issue - ignore cleanup failure
                pass
    
    @patch('book_triage.vision.OpenAI')
    @patch('book_triage.vision.pytesseract.image_to_string')
    def test_extract_title_from_image_fallback_to_tesseract(self, mock_tesseract, mock_openai):
    ...
    @patch('book_triage.vision.OpenAI')
    @patch('book_triage.vision.pytesseract.image_to_string')
    def test_extract_title_and_isbn_from_image_success(self, mock_tesseract, mock_openai):
    ...
    def test_extract_title_and_isbn_tesseract_fallback(self, mock_tesseract):
        ...
        try:
            mock_tesseract.return_value = "Machine Learning Basics\nPublished 2023\n9780987654321\nPage 1"
            
            processor = VisionProcessor(use_openai_vision=False)
            title, isbn = processor.extract_title_and_isbn_from_image(image_path)
            
            assert title == "Machine Learning Basics"
            assert isbn == "9780987654321"
        finally:
            try:
                image_path.unlink()
            except (PermissionError, OSError):
    ...
    def test_extract_title_and_isbn_nonexistent_file(self):
        """Test title and ISBN extraction with non-existent file."""
        processor = VisionProcessor(use_openai_vision=False)
    ...
    def test_extract_title_and_isbn_no_isbn_found(self, mock_tesseract):
        """Test title and ISBN extraction when no ISBN is found."""
        with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp:
            img = Image.new('RGB', (100, 100), color='white')
            img.save(tmp.name, 'JPEG')
            image_path = Path(tmp.name)
        ...
        try:
            mock_tesseract.return_value = "Book Without ISBN\nAuthor Name\nPublisher 2023"
            
            processor = VisionProcessor(use_openai_vision=False)
            title, isbn = processor.extract_title_and_isbn_from_image(image_path)
            
            assert title == "Book Without ISBN"
            assert isbn == ""
        finally:
            try:
                image_path.unlink()
    ...
    def test_image_format_conversion(self):
        """Test image format conversion in Tesseract extraction."""
        ...
        try:
            with patch('book_triage.vision.pytesseract.image_to_string') as mock_tesseract:
                mock_tesseract.return_value = "Test Title"
                
                processor = VisionProcessor(use_openai_vision=False)
                title = processor.extract_title_from_image(image_path)
                
                assert title == "Test Title"
                # Verify that pytesseract was called (image was processed)
                mock_tesseract.assert_called_once()
        finally:
            try:
                image_path.unlink()
...
class TestVisionProcessorIntegration:
    ...
    def test_processor_with_real_image_mock_ocr(self):
        """Test processor with a real image but mocked OCR."""
